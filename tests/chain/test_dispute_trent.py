...
def test_seller_agree(sAgent):
    sAgent.confirm_dispute(order_id, False)
...
def test_trent_fetch(tAgent):
    order_id = tAgent.fetch_unhandled_disputes(0, tAgent.get_order_num()+1)[-1]
    tAgent.handle_dispute(order_id, True, False, True)

    test_record = tAgent.query_order(order_id)
    assert test_record[10] == 5

    test_dispute_record = tAgent.fetch_dispute_result(order_id)
