...
class BuyerTrans(Trans):
    ...
    def place_order(self, order_info: models.OrderInfo) -> "order id":
        transfer_filter = self.contract.on('TransInitiated', {'filter': {'_from': self.web3.default_account}})
        # Initiate a transaction
        offered_price = self.ONE_ETH_IN_WEI * order_info.value
        transaction = {'value': offered_price, 'from': self.web3.default_account}
    ...
    def withdraw_order(self, order_id):
        transaction = {'value': 0, 'from': self.web3.default_account}
...
def create_trans(cls, chain, contract_name):
    return cls(chain, contract_name)
